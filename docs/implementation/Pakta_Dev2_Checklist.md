...
> **Cambio de stack (23 sep 2026):** el proveedor actual es **NVIDIA NIM** (API compatible con OpenAI). La arquitectura es agnóstica de proveedor: `packages/ai-extraction/src/extractInvoice.ts` define `InvoiceExtractor` (texto → JSON crudo), mientras el proveedor real vive en `src/providers/nvidia.ts`. Cambiar de proveedor requiere añadir una implementación, no reescribir el pipeline.
...
| HU-D2-15 | Flujo de reverificación de wallet cuando cambia | Módulo Vendors & Wallets: wallet pendiente → confirmación → `attestationStatus=ATTESTED` | Test de integración del flujo completo | ✅ `packages/db` (PostgreSQL vía Neon) + `POST /vendors/:id/wallet` y `/wallet/attest` en `apps/api` + botones reales en `apps/web`. Flujo probado de punta a punta: INV-004 pasa VENDOR_WALLET_CHANGED → UNATTESTED_WALLET → READY sin tocar código. |

**Sprint Review 2:** una factura PDF real entra por AI Extraction y produce un `CanonicalPayable` que el kernel evalúa exactamente igual que uno salido del workbook. La validación de NVIDIA NIM está cubierta por la prueba de integración opcional, que se ejecuta solo cuando existe `NVIDIA_API_KEY`.

**Extra no planificado en el doc original:** `apps/web` pasó de mockup a dashboard conectado: Intake, Payables, Vendors & Wallets, Exceptions y Proof-of-Payable leen `apps/api`; no quedan arrays mock como fuente de datos.
...
**Siguiente paso recomendado:** cerrar el hand-off de `ProofOfPayable` con el Settlement Adapter y reconciliar los eventos on-chain de vuelta en la API.
